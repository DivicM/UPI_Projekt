...
/*document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function fetchAndRenderGrades(dataType) {
    try {
  ...
  saveGradesButton.addEventListener("click", async () => {
    ...
    let rows = [];
      ...
      const response = await fetch(`http://localhost:5000/${subjectName}/update`, {
          ...
          body: JSON.stringify({ username: studentEmail, dataType: "grades", data: rows }),
      });

      const result = await response.json();
      alert(result.message);
      await fetchAndRenderGrades("grades"); // Ponovno učitavanje nakon spremanja
  });
   ...
   async function deleteRow(index, dataType) {
      ...
      try {
          const response = await fetch(`http://localhost:5000/${subjectName}/delete/${dataType}/${index}`, {
              method: "DELETE",
              headers: {
                  "Authorization": `Bearer ${token}}`,
                  "Content-Type": "application/json",
